...
class Bot(threading.Thread):
	...
	# get_member_nick 获取群聊成员昵称
	def get_member_nick(self, roomid, wxid):
		# 获取指定群的成员的昵称 或 微信好友的昵称
		uri='api/getmembernick'
		data={
			'type':query.CHATROOM_MEMBER_NICK,
			'wxid':wxid,
			'roomid':roomid or 'null'
		}
		return json.loads(self.__send_http(uri,data)['content'])['nick']
	...
	def wshd_recv_txt_msg(self, msg_json):
		...
		nickname = self.get_member_nick(roomid, senderid)
		key_replys = {
			"ding": "dong",
			"dong": "ding",
		}
